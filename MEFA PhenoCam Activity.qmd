...
```{r}
# Choose site to explore; change these later when playing around with data
# Enter name exactly as it appears in the table/on the website
site_name = "stanislausforage"

# Indicate vegetation type (listed on site page)
veg = "AG"  #decidious broadleaf=DB, that's what we did before
...
GCC_timeseries <- get_pheno_ts(site = site_name, 
                               vegType = veg, 
                               roiID = ROI, 
                               type = '3day')  #data is segregated by Phenocam to be either 1day or 3day
...
ggplot(GCC_timeseries, aes(x = date, y = gcc_90)) +
  geom_line(color = "darkgreen") +
  geom_point(color = "darkgreen") +
  labs(
    x = "Date",
    y = "GCC",
    title = paste0(site_name, ": ", "Canopy Greenness Timeseries")
  ) +
theme_minimal()+
  theme(
...
download_phenocam(site = site_name,
                  ...
                  frequency = 3,
                  phenophase = TRUE,
                  out_dir = dir) #save to our directory called pheno_images
